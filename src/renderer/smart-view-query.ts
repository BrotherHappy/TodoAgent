...
const findNamedValue = (
  ...
  const explicit = normalizedQuery.match(
    new RegExp(`${normalizedPrefix}\\s*:\\s*([^ ]+)`, "u"),
  )?.[1];
  if (explicit) {
...
export const parseSmartViewQuery = (
  ...
  const project = findNamedValue(query, options.projects ?? [], "项目");
  const tag = findNamedValue(query, options.tags ?? [], "标签");
  const context = findNamedValue(query, options.contexts ?? [], "情境");
  if (project.unknown) {
    return {
      kind: "error",
