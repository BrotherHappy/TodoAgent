...
Todo Agent 用一只原创的 Todo Pet 替代传统悬浮球和胶囊：它常驻桌面、轮播今天的任务，也能展开为任务面板、番茄钟、流式 Agent 对话和小窝。宠物默认使用 GPT Image 生成并规整的透明高帧动作图集：主动作是每组 433 个播放帧（10 个原始姿势 + 每段 47 个离线轮廓插帧），互动动作是每组 577 个播放帧（13 个原始姿势 + 每段 47 个离线轮廓插帧），摸头使用连续的 0→6 姿势段，避免手部在循环中凭空消失。运行时图集按 16 列分页纵向堆叠（每格 128px，宽度固定 2048px），避开 macOS WebKit 超宽纹理的隐式切片；播放器在 Canvas 中按设备实际 60/90/120Hz 刷新率精确裁帧，高刷屏不再重复展示同一格，只用一次 `copy` 合成替换完整 cell，并以正向/反向往返播放。源矩形会避开一像素邻格采样，离线插帧同时对身体和道具做轮廓/颜色连续化，消除中点换色造成的撕裂；两层 Canvas 采用隐藏缓冲后再切换，动作切换再用 5 个显示节拍的完整像素交接，图集解码期间保留同动作 SVG 后备姿态，避免低帧停顿、空白闪烁、跨帧重影和硬切。图集不叠加第二套大幅位移，SVG 交互覆盖层与降级路径仍然保留。没有账号、网络或模型时，本地任务、专注、提醒、成长与日记依然可用。
