...
Agent 能力在“设置 → 权限中心”分成五层：任务读写、飞书同步、网页研究、文件与终端、剪贴板与屏幕。每层都能独立关闭；关闭后对应工具不会出现在下一次模型运行中，飞书关闭时也不会降级为本地写入。主 Agent 和 Todo Pet 小窗的 Markdown 回复支持手动“朗读/停止朗读”，使用本机系统语音合成，不自动播放、不上传、不保存音频。
