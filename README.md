...
| 日历与排程 | 时间线可导入本地 `.ics`，日视图显示只读议程、周视图按日期显示会议数量与占用时长；会议卡可打开预填的会后跟进任务，或从 `DESCRIPTION` 中提取明确的行动项/待办/下一步，在本地预览中逐项编辑、勾选后批量创建；保存前仍由用户确认，且只创建本地任务；今日规划会扣除会议占用并避开忙碌区，支持全天/跨午夜事件、清空与撤销；不会写回外部日历或飞书 |
