...
Todo Agent 用一只原创的 Todo Pet 替代传统悬浮球和胶囊：它常驻桌面、轮播今天的任务，也能展开为任务面板、番茄钟、流式 Agent 对话和小窝。宠物默认使用 GPT Image 生成并规整的透明高帧动作图集：主动作是每组 577 个播放帧（10 个原始姿势 + 每段 63 个密集过渡帧），互动动作是每组 769 个播放帧（13 个原始姿势 + 每段 63 个密集过渡帧），摸头使用连续的 0→6 姿势段，避免手部在循环中凭空消失。v24/v22 过渡帧以稳定下半身锚点做清晰轮廓过渡，颜色差异较大的手、眼睛和尾巴按单一轮廓择一呈现，绳子、任务卡和星光等独立道具保持整层交接，因此不会出现彩边或双影。运行时图集按 16 列分页纵向堆叠（每格 128px，宽度固定 2048px），避开 macOS WebKit 超宽纹理的隐式切片；播放器在 Canvas 中按设备实际刷新率推进，并把有效节拍取为 `max(8ms, 实测刷新周期)`，每次刷新最多呈现一格相邻帧，遇到忙碌时宁可短暂放慢也不瞬移。每次只用一次 `copy` 合成替换完整 cell，并以正向/反向往返播放。源矩形会避开一像素邻格采样；两层 Canvas 采用隐藏缓冲后再切换，动作切换保留上一张完整帧，图集解码期间保留同动作 SVG 后备姿态。悬浮窗关闭后台渲染降频，失焦时仍按显示时钟绘制；流式 Agent 和气泡更新不会重建宠物动画层。没有账号、网络或模型时，本地任务、专注、提醒、成长与日记依然可用。
