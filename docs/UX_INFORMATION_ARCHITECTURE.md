...
| Todo Pet 档案迁移 | P1（已落地 v1.74） | 设置 → 数据与安全提供“导出宠物档案”和“导入并预览”；预览展示成长、库存、冒险、小游戏、日记、记忆、主动消息和专注历史数量 | 导入先选择保留本机或覆盖本机；当前运行中的专注始终保留，凭据和本地路径不进入档案；覆盖只更新宠物档案，不创建任务、不触发飞书同步 |
