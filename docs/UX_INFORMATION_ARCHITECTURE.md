...
| 任务自动化 | P1（已落地 v2.43） | 设置 → 任务自动化提供“触发器 / 条件 / 动作”规则编辑器与启用开关；列表显示规则名称、匹配来源、动作摘要和删除入口；任务详情的更多操作提供手动规则按钮 | 最多 50 条；支持新建/完成事件和手动应用，本地私有字段动作，保存前做白名单校验；手动按钮在条件不匹配或已是目标状态时禁用；执行生成普通任务历史与撤销，不执行脚本、不联网、不创建任务、不写回飞书 |
