...
| 分组标题 | P1（已落地 v2.38–v2.39） | 新建任务、任务详情和 Agent 可写入本地标题；非手动 Today 排序时按标题聚合、显示数量并可折叠/展开；折叠按任务视图和来源记忆 | 未填写进入“未分组”；标题与折叠状态均为本地投影，不创建额外实体，不与飞书 tasklist/section 混用，飞书任务只保留本地显示 |
