...
| 周视图日历负载 | P1（已落地 v1.71） | 周概览在有事件的日期标题下显示会议数量与占用时长，顶部摘要显示本周会议总负载 | 只读聚合同一日历缓存；点击日期进入日视图查看完整议程；无事件日期不显示空标签，不创建任务、不写回日历或飞书 |
...
| 项目提醒批量策略 | P1（已落地 v1.72） | 设置 → 提醒 → 项目例外支持多选项目，并一次应用“跟随来源 / 全部提醒 / 仅高优先级 / 关闭” | 按住 Cmd/Ctrl 多选项目后点击“应用到已选”；跟随来源会删除项目例外，未选项目保持原样；只写本地普通提醒策略，不改变任务、晨报、同步或飞书字段 |
| 时间线专注节奏 | P1（已落地 v1.56） | 周视图显示 7 日专注柱状节奏、总分钟、专注段数、平均时长和投入最多任务 | 点击任务回到详情；只读投影，空状态不施压，不修改任务或飞书 |
| 专注守护 | P1（已落地 v1.77） | 设置中维护最多 12 个应用名片段；专注期间只读取前台应用名，匹配后显示可折叠宠物气泡或暂停本次专注 | 关闭 / 温和提醒 / 自动暂停三档；不读取窗口标题或内容，不关闭、阻挡或控制其他应用；支持暂时忽略，服从会议、全屏和安静边界 |
...
| 重点标记 | P1（已落地 v2.36） | 任务行与详情提供低干扰的星标按钮；筛选弹窗、一句话筛选、保存视图、批量编辑和 Todo Pet 视图支持“只看重点” | 标记只表达“需要留意”，不替代优先级、不改变排序事实；飞书任务也只写本地私人字段并显示边界提示，Agent 修改先预览确认 |
| 稍后安排 | P1（已落地 v2.37） | 任务行、新建/详情和 Agent 支持设置本地可用日期；侧栏提供“稍后安排”集合，到期前从 Today 隐藏 | 未来日期前显示“稍后至 YYYY-MM-DD”，可一键恢复可安排；到期后回到普通开放集合；飞书任务只写本地私人字段，不改变飞书截止、状态或同步 |
| 分组标题 | P1（已落地 v2.38–v2.40） | 新建任务、任务详情和 Agent 可写入本地标题；非手动 Today 排序时按标题聚合、显示数量并可折叠/展开；折叠按任务视图和来源记忆；筛选、保存视图、Todo Pet 和 Agent 查询可精确使用标题 | 未填写进入“未分组”；标题、折叠状态和筛选条件均为本地投影，不创建额外实体，不与飞书 tasklist/section 混用，未知标题不套用，飞书任务只保留本地显示 |
| Todo Pet 智能视图 | P1（已落地 v1.73） | 宠物任务面板在“全部”页显示主窗口已保存的智能视图选择器，可直接切换项目、标签、情境、日期、来源和排序组合 | 选择只改变宠物面板的本地投影；首次默认全部任务，当前选择随宠物窗口记住；视图被删除或失效时回退全部任务；任务仍来自同一快照，不复制、不写入任务或飞书 |
...
| Todo Pet 档案迁移 | P1（已落地 v1.74 / v1.75） | 设置 → 数据与安全提供“导出宠物档案”和“导入并预览”；预览展示成长、库存、弹性习惯、冒险、小游戏、日记、记忆、主动消息和专注历史数量 | 导入先选择保留本机或覆盖本机；当前运行中的专注始终保留，凭据和本地路径不进入档案；覆盖只更新宠物档案，不创建任务、不触发飞书同步；旧档案缺少习惯字段时自动补默认值 |
...
| 小窝弹性习惯 | P1（已落地 v1.75） | 默认喝水、伸展、收尾提示，以及最多 12 项可自定义名称、提示和间隔的生活节奏卡 | 完成一次、稍后、暂停/恢复、编辑、移除；由 PetService 本地持久化并随档案迁移，不计连续、不扣成长、不创建任务、不进入飞书 |
| 小窝每周同行目标 | P1（已落地 v1.76） | 最多 3 张可选目标卡，追踪完成任务、专注分钟或习惯照顾；进度来自真实事实快照 | 新增、编辑、暂停/恢复、移除；本周 Monday-first 日期范围；不复制任务、不维护第二份计数、不制造连续签到或惩罚，不写回飞书 |
...
| 小窝引导式回顾 | P1（已落地 v1.78） | 在宠物回顾事实卡中主动开始临时会话，每次只展示一项任务与逾期/阻塞/待排原因 | 完成、安排今天、开始专注、打开任务、稍后或结束；会话不创建副本、不要求清空，写入复用任务控制器、撤销和同步 |
...
| 提醒原因解释 | P1（已落地 v2.41） | 处理提醒时默认展开“为什么现在提醒”，显示“你设置的提醒 / 截止时间 / 晨间简报 / 合并错过提醒”等本地事实原因 | 原因由确定性本地规则生成，不调用模型、不把外部任务正文当作系统解释；继续提供完成、打开、稍后和今天不再提醒 |
| 任务自动化 | P1（已落地 v2.44） | 设置 → 任务自动化提供“触发器 / 条件 / 动作”规则编辑器与启用开关；列表显示规则名称、匹配来源、动作摘要和删除入口；任务详情的更多操作提供手动规则按钮，批量选择工具栏提供规则选择、可应用数量和预览确认 | 最多 50 条；支持新建/完成事件和手动应用，本地私有字段动作，保存前做白名单校验；单任务/批量按钮在条件不匹配或已是目标状态时禁用；批量执行由主进程校验所有基线并原子提交，生成普通任务历史与单次撤销，不执行脚本、不联网、不创建任务、不写回飞书 |
