...
任务列表的单项 Agent 入口由 renderer 的纯函数 `buildTaskAgentPrompt` 生成草稿上下文（任务标题、稳定本地 ID、状态和来源）。入口只调用已有的 `askAgent` 导航与草稿消费路径，不新增任务写入或绕过权限；用户发送后仍从任务工具查询详情，并经过既有数据范围、风险预览、确认、审计和撤销链。
