...
- Todo Pet 档案由 `PetDataPortabilityService` 使用独立 `.todo-pet.json` 格式迁移，稳定状态通过 `PetService.portableSnapshot` / `replacePortableSnapshot` 读取和写入。导入先生成带 digest 的预览，仅支持“保留本机”或“覆盖本机”；覆盖不信任导入 revision，沿用本机 mutation 队列递增修订号，并保留现有运行中的 focus。备份只包含成长、外观、库存、弹性习惯、本周同行目标、房间伙伴、冒险、小游戏、日记、记忆、主动消息和 focusHistory，不写入凭据、localPath、任务或飞书映射；宠物档案写入独立原子文件，不参与任务/设置/审计事务。
- `PetProfile.personality` 是六种本地预设（`gentle` / `energetic` / `calm` / `playful` / `witty` / `quiet`），由 `PetService` 校验、原子持久化并随宠物档案迁移；旧档案缺失或短暂预发布别名回退到 `gentle`。`PetCharacter` 与浮窗/主窗口/小游戏都读取同一 profile，用动作节奏和无障碍标签表达性格，不进入任务、提醒、权限或飞书同步字段。
- `PersonaSettings.syncWithPet` 默认开启，Agent 每轮只通过 `AgentDesktopServiceOptions.getPetPersonality` 读取当前 `PetProfile.personality` 的枚举值，再映射为固定的表达规则；不传输宠物成长、记忆、任务或外观内容。用户关闭后不读取该值，Agent 使用独立的四种 Persona preset。同步人格只改变 system prompt 的语气/主动程度说明，不改变模型数据范围、权限引擎、工具列表、任务字段或 Feishu payload；旧设置缺少该字段时 `SettingsService` 回退为 `true`。
- Agent 运行事件中的 `tool-proposed`、`approval-required`、`tool-started` 和 `tool-finished` 由 renderer 投影成短期 `AgentToolActivity` 执行卡，按 invocation/provider call 关联并只保留最近 12 项；卡片展示工具族、风险、状态和受信任的 `EffectPlan.preview`，不持久化、不进入模型 history，也不替代主进程审计。权限拒绝同样发送 `tool-finished(status=denied)`，避免 UI 停留在“已拟定”而无法解释最终结果。
- `PetState.companions` 是最多 3 个、只属于小窝的 `PetCompanion` 记录；`PetService` 对伙伴种类、名称、性格、唯一性和数量做白名单校验，旧档案缺失时回退空数组，新增/编辑/移除沿用同一原子 mutation。`PetCompanionAvatar` 只渲染伙伴的轻量 CSS 形象，不读取任务快照、不复制任务、不进入提醒预算、权限判断、Agent 上下文或 Feishu payload；伙伴随 `.todo-pet.json` 一起迁移，桌面浮窗仍只使用主 `PetProfile`。
- `PetAppearance.decorationPositions` 是可选的房间摆件布局覆盖，使用 `src/shared/pet-room-layout.ts` 的已知 ID、默认坐标和边界（x/y 百分比与 75%–130% 缩放）；`PetService` 与 `petCustomize` IPC 共同校验、收敛并原子保存，旧档案缺失时不增加字段且 renderer 投影默认布局。`PetRoomLayoutControls` 提供方向/缩放控制，房间舞台支持 pointer 拖动并在 pointerup 时只提交一次完整布局；两者都只发送本地外观 patch，不创建任务、不进入 Agent 上下文、不写入 Feishu payload；未知 ID、NaN、越界坐标和过大缩放均回退到安全范围。
- `PetAppearance.atmosphere` 是可选的小窝视觉氛围（`daylight` / `cozy` / `moonlit`）；旧档案缺失时 renderer 使用 `daylight`，`PetService` 与 `petCustomize` IPC 只接受白名单值并原子保存。氛围仅由 CSS 改变房间背景光线和窗户色彩，不自动播放音频、不读取天气或系统时间，不进入任务、提醒、Agent 上下文或 Feishu payload。
...
- Agent 能力分层由 `src/shared/agent-capabilities.ts` 维护唯一工具族映射。`AppSettings.agentCapabilities` 默认全开，权限中心可独立关闭任务读写、飞书同步、网页研究、文件与终端、剪贴板与屏幕；主进程创建 `ToolRegistry` 时先按分层过滤模型工具，并在 `prepare` / `execute` 再次校验开关，防止运行中撤权继续调用。任务工具还在读取/创建/远端字段写回前检查 Feishu 能力，关闭时不会把远端操作降级为本地操作；该设置只影响本机工具可见性，不改变 `modelDataScope`、风险审批或飞书 payload。
...
- 批量任务动作通过 `tasks:apply-bulk-action` 暴露为受限的判别联合（complete / reopen / move-to-today / trash / restore / edit）。渲染层先收集精确任务 ID 和 `updatedAt` 基线，预览确认后主进程在一次 LocalStore 事务中先验证全部目标、角色、会签、回收站状态和基线，再写入一个 `bulk` operation；批量编辑允许优先级、重点标记、项目、清单和标签，标签支持替换/追加/移除；完成循环任务时生成的下一次任务也纳入同一 operation。普通任务和飞书任务沿用原有 `applyPatch` / `markSync` 边界，Today 安排以及批量属性编辑只写私人计划/上下文字段，不把本地重点标记、项目、清单或标签送入飞书；撤销复用快照冲突保护，不能把后续本地编辑或远端拉取覆盖掉。
- 重复任务的“跳过本次”通过 `tasks:skip-recurring` 暴露为单任务变更。`TaskService.skipRecurringTask` 只接受开放的本地循环任务，在一个事务内用 `getNextOccurrence` 计算下一锚点，并用 `shiftTemporal` 同步平移 `plannedDate`、开始/截止时间、时间块和提醒，递增 `recurrenceIndex` 后记录 `skip-recurring` operation；任务 ID、内容、系列和同步身份不变，不创建影子任务。飞书循环、专注中的任务、无日期锚点和已到末次都 fail-closed；撤销沿用快照冲突保护，导入/Markdown 历史只暴露操作类型，不扩展飞书 payload。
- Agent 的 `task_skip_recurring` 只接受明确任务 ID；分析阶段重新读取本地循环任务，用同一 `getNextOccurrence` 生成当前/下一次锚点预览，风险固定为 R1、`remoteWrite=false`，执行阶段再次读取后调用 `TaskService.skipRecurringTask`，返回新任务快照和单一 undo operation。任务能力开关、运行中撤权、基线与审计仍由既有工具注册表/权限引擎承接，飞书能力关闭或任务属于飞书时不会降级为本地跳过。
...
- 保存视图由 `src/renderer/smart-views.ts` 以版本兼容的本地 JSON 保存；除优先级、项目和来源外，v1.33 增加精确标签与日期范围（逾期、今天、未来 7 天、无日期），v1.41 增加手动情境条件，v2.36 增加本地重点标记条件。读取旧视图时补齐 `tag=all`、`context=all`、`dateFilter=any` 与 `flagged=false`，过滤只对当前任务快照做确定性投影，不修改任务、不增加同步载荷，也不把筛选条件发送给 Agent；一句话筛选同样只在本地解析“重点任务/已标记”等明确表达。
- 任务 `deferUntil` 是新的本地可用日期字段：未来日期前 `today` 视图排除该开放任务，`deferred` 视图只展示仍在等待的任务，到期后任务重新回到普通开放集合；如果任务同时有私人计划/截止日期，未来 defer 日期优先作为 Upcoming 的可用时间。字段由 TaskService 用 `assertLocalDate` 校验，进入操作历史、JSON/Markdown 导出与导入，Feishu 共享快照、适配器和写回 payload 均明确排除，因此飞书任务只改变本地可见性，不会改动飞书截止时间或同步状态。新建/详情/Agent `task_create`、`task_update`（含 `clearFields: ["deferUntil"]`）和 `task_list(view="deferred")` 共用这一边界。
...
- 分组标题沿用任务已有的本地 `sectionId` 字段，不新增实体或 IPC。TaskService 在写入时裁剪并限制为 80 字，渲染层在非手动 Today 排序下用稳定的首次出现顺序把任务聚合成标题组；Agent 的 `task_create` / `task_update` / `task_bulk_create` 可预览、设置或清除该字段。它与 `source.tasklist` 的飞书绑定严格分离，永不进入 Feishu Task v2 payload；JSON/Markdown 导出沿用现有任务字段白名单。
- 分组标题的折叠状态属于 renderer 视图偏好，按 `TaskView + sourceFilter` 作用域存储在受限 localStorage 中（最多 24 个作用域、每个作用域 80 个分组）。标题按钮通过 `aria-expanded` / `aria-controls` 暴露状态；折叠只隐藏当前列表投影，不改变任务快照、计数、筛选、Agent 上下文或飞书同步，存储不可用时回退内存状态。
- 分组标题筛选沿用 `TaskFilter.sectionIds` 的本地精确匹配；保存视图在兼容旧数据的前提下增加可选 `sectionId`，Filter Assist 只从当前快照的标题选项中解析 `分组：标题`，Todo Pet 复用保存视图过滤，Agent `task_list` 通过可选 `sectionId` 参数过滤。该值不参与 Feishu Task v2 payload，未知标题拒绝套用而不猜测远端 ID。
...
- 最近上下文：`src/renderer/context-capture-history.ts` 使用独立的 renderer `localStorage` 键保存用户在快速录入或 Todo Pet 小窗的剪贴板、当前窗口、选中文本或文本/链接拖入预览卡中明确点击“保存到最近上下文”的内容；最多 12 条，单条原文最多 2,000 字，快速录入空状态提供回用与清空，宠物小窗提供保存成功反馈。它不自动记录、不保存文件/图片正文，不建立窗口监听，不进入任务事件、Agent、飞书、`.todo-pet.json` 或安全导出；存储失败只降级为当前捕获继续可用。
- Agent 回复复用：`AgentMarkdown` 对已完成的 Markdown 提供显式“保存到最近上下文”和“复制 Markdown”两个本地出口；前者写入同一 `agent-reply` 历史并遵守 12 条/2,000 字边界，后者只调用用户点击触发的 `navigator.clipboard.writeText`。流式内容、空内容和不支持剪贴板的环境均禁用，复制失败静默保持回复可读，不进入任务、飞书、审计或模型上下文。
- Agent 上下文回用：`CONTEXT_CAPTURE_HISTORY_CHANGED_EVENT` 在本机历史保存/清空后通知已挂载的 Agent 和 Todo Pet 界面；两处只在用户点击历史条目时把截断后的原文追加到输入框，不自动发送、不改变会话消息、不重复写入历史。组件卸载会移除监听，读取失败按空历史降级。
- Agent 行动项提取：`extractActionItemsFromText` 复用会议行动项的确定性规则，只接受明确 cue、清单或动作动词并去重/限制最多 8 项；`AgentActionItemsSheet` 只展示可编辑草稿，确认后调用普通本地 `TaskController.create`，不进入 Agent 工具调用、不写入飞书。网页研究内容在面板中继续标注为不可信参考，提取本身不会执行指令。
- Agent 研究卡确认：当主 Agent 当前存在选中任务时，`AgentMarkdown` 才显示“保存研究卡”入口；`AgentResearchCardSheet` 将回复预填为可编辑标题、首个安全 http/https 来源、原始 Markdown 摘要和最多 20 条行动项，用户确认后复用 `TaskController.update` 写入该任务的 `researchCards`。组件与父层再次校验任务仍存在、卡片数量、字段长度和 URL 无凭据；流式回复、无选中任务、任务切换/删除或超限均 fail-closed。该路径是本机私人上下文，不经过 Agent 工具、权限批准、飞书 adapter 或网页执行链，来源和摘要不会被隐式发送到模型。
- 研究卡可读渲染：`ResearchCardMarkdown` 在任务详情中用 `react-markdown` + GFM 只读渲染研究摘要；链接必须是安全的 http/https 并通过 `desktopApi.shell.openExternal` 打开，原始 HTML 由 `skipHtml` 丢弃，远程图片改为替代文本，表格使用独立横向滚动容器，代码/引用/清单仅作为视觉内容。渲染不保存、不执行、不调用 Agent，不改变 `TaskResearchCard` 或飞书 payload。
- 研究卡行动项转任务：`ResearchCardActionList` 将每条行动项作为显式本地创建按钮；`buildResearchActionTaskInput` 只生成 `source: local`、`sync: local` 的 `CreateTaskInput`，保留研究卡标题、来源链接和原任务标题作为备注 provenance，并继承原任务的私人项目/计划日期。创建复用 `TaskController.create({selectCreated:false})`，失败只保留当前项可重试；组件在当前详情会话内用 `cardId:index` 禁用已成功项，防止重复点击，不建立第二事实层、不写回飞书、不调用模型或跳过权限/审计链。
- 语音输出：`src/renderer/speech-output.ts` 将 Markdown 或气泡文本确定性裁剪为本地朗读文本（代码块替换为“代码块已省略”、链接只读标题，最多 4,000 字），共享 `SpeechOutputButton` 被主 Agent、Todo Pet 小窗、晨间简报、当前任务气泡和专注气泡复用。全局只允许一个本机 utterance，新的任务/摘要文本出现时先停止旧 utterance；隐私模式在上层生成不含任务标题的提示。使用浏览器/Electron 本机 `speechSynthesis`，不自动播放、不保存音频、不上传；API 不可用时按钮禁用，朗读失败只恢复文字显示。
- Agent 研究回复回用：`AgentMarkdown` 在非流式完成回复上提供显式“保存上下文”，将原始 Markdown 作为 `agent-reply` 写入 `context-capture-history`；标签只取受限的首段摘要，历史仍共享最多 12 条/单条 2,000 字上限，快速捕获和宠物入口可继续回用。流式内容不显示可操作保存结果，不自动记录、不进入会话以外的模型上下文、不写任务/飞书，清空沿用同一 localStorage 删除入口。
...
- 工作流模板：渲染进程只保存版本化模板 JSON；模板步骤仅允许任务标题、备注、标签、优先级、预计时长、有限的相对日期和前置步骤引用，变量只支持 `title/date/now`。快速捕获先生成确定性预览，再逐项写入本地或飞书；不会执行模板脚本、后台定时器或隐式外部调用。任务详情的“保存为工作流模板”默认生成一个 `defaultSource: local` 步骤，可选择带入最多 11 个子任务；本地使用时按已创建步骤 ID 恢复父子关系，飞书来源永不把本地父任务 ID 传入远端。模板带入备注、标签、优先级和有效估时；完成状态、具体日期、附件、同步身份、Feishu 外部 ID 与凭据不会复制，未知模板变量的备注安全舍弃并在 UI 说明边界。Quick Capture 解析 `@情境` 与 `情境：情境` 为本地 contexts，并把解析结果作为可编辑 chip 展示；同时把 `预计 45 分钟`、`用时 1 小时`、`30m/1h` 标准化为 5–720 分钟的本地 `estimatedMinutes`，把 `每天`、`每个工作日`、`每周一/三/五`、`每月15日` 和 `每隔 2 周` 转成受限的本地 `RecurrenceRule`，均在保存前显示 chip。解析不触发定位或后台监听；模板批量创建仍使用各步骤自己的估时定义，快速捕获循环只写入非模板本地任务。
...
- 提醒原因投影：`ReminderProjector` 为显式本地提醒、截止时间和晨间简报生成闭合枚举的 `ReminderReason`，`ReminderScheduler` 为多项错过提醒生成合并原因并原样传到 `ReminderDelivery`；原因只来自本地规则，不读取模型、不执行外部文本指令。主窗口处理面板以可折叠“为什么现在提醒”展示标签和下一步说明，原因字段不进入任务、飞书 payload、模型上下文或提醒运行状态。
- 受限任务自动化：`src/shared/task-automations.ts` 定义版本无关的“触发器 → 条件 → 私有动作”白名单，设置服务对规则逐条 fail-closed 并限制最多 50 条；`TaskAutomationService` 接收主进程串行维护的前后任务快照，只识别新建和开放→完成边沿，按规则顺序调用 `TaskService.updateTask`，`manual` 规则不会被后台事件触发。任务详情和批量选择入口复用同一条件匹配器与私有补丁生成器；单任务走普通 `update` operation，批量入口由 `TaskService.applyTaskAutomation` 在一个事务内重新核对启用的 canonical rule、每项条件和 `updatedAt` 基线，再生成单一 `bulk` operation，因此可整体撤销且不会半批量写入。动作仅覆盖重点标记、项目/清单/分组、稍后日期、标签和情境，均不触发 `patchHasRemoteImpact` 的飞书共享字段；规则执行不创建任务、不联网、不调用模型，失败只记录本地可解释错误，且不会递归派发任务变更。
