...
- 受限任务自动化：`src/shared/task-automations.ts` 定义版本无关的“触发器 → 条件 → 私有动作”白名单，设置服务对规则逐条 fail-closed 并限制最多 50 条；`TaskAutomationService` 接收主进程串行维护的前后任务快照，只识别新建和开放→完成边沿，按规则顺序调用 `TaskService.updateTask`，`manual` 规则不会被后台事件触发。任务详情的手动入口复用同一条件匹配器和私有补丁生成器，满足条件后走普通 `update` operation，因此可单次撤销。动作仅覆盖重点标记、项目/清单/分组、稍后日期、标签和情境，均不触发 `patchHasRemoteImpact` 的飞书共享字段；规则执行不创建任务、不联网、不调用模型，失败只记录本地可解释错误，且不会递归派发任务变更。
