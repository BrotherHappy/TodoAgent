...
- 批量任务动作通过 `tasks:apply-bulk-action` 暴露为受限的判别联合（complete / reopen / move-to-today / trash / restore / edit）。渲染层先收集精确任务 ID 和 `updatedAt` 基线，预览确认后主进程在一次 LocalStore 事务中先验证全部目标、角色、会签、回收站状态和基线，再写入一个 `bulk` operation；批量编辑只允许优先级、项目、清单和标签，标签支持替换/追加/移除；完成循环任务时生成的下一次任务也纳入同一 operation。普通任务和飞书任务沿用原有 `applyPatch` / `markSync` 边界，Today 安排以及批量属性编辑只写私人计划/上下文字段，不把本地项目、清单或标签送入飞书；撤销复用快照冲突保护，不能把后续本地编辑或远端拉取覆盖掉。
