...
- `PetAppearance.decorationPositions` 是可选的房间摆件布局覆盖，使用 `src/shared/pet-room-layout.ts` 的已知 ID、默认坐标和边界（x/y 百分比与 75%–130% 缩放）；`PetService` 与 `petCustomize` IPC 共同校验、收敛并原子保存，旧档案缺失时不增加字段且 renderer 投影默认布局。`PetRoomLayoutControls` 只发送本地外观 patch，不创建任务、不进入 Agent 上下文、不写入 Feishu payload；未知 ID、NaN、越界坐标和过大缩放均回退到安全范围。
