...
- 语音输出：`src/renderer/speech-output.ts` 将 Agent Markdown 确定性裁剪为本地朗读文本（代码块替换为“代码块已省略”、链接只读标题，最多 4,000 字），`AgentMarkdown` 在主窗口与宠物小窗提供显式朗读/停止按钮。使用浏览器/Electron 本机 `speechSynthesis`，不自动播放、不保存音频、不上传；API 不可用时按钮禁用，朗读失败只恢复文字显示。
