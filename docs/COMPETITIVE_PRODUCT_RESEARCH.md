...
23. 多日历来源控制：吸收 Morgen / Any.do / Structured 的多日历聚合与“在同一时间线中按来源聚焦”习惯。时间线现在把本地 `.ics` 导入事件按 `sourceName` 聚合，用户可以独立显示/隐藏工作与个人等来源，隐藏偏好只保存在本机且不改变原始事件；若用户提供日历写入回调，还可只清除一个来源并通过一次撤销恢复，避免为了暂时整理视图而误删其它日程。筛选只影响时间线和周负载投影，Today 规划继续使用完整事件事实来保护容量，不写回外部日历、飞书或任务共享字段。
