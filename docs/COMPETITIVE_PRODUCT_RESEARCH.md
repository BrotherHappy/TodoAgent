...
#### 2.3.1 本轮桌面迁移：可逆的专注布局

Things 的官方桌面快捷键将“隐藏 / 显示侧栏”作为低成本的全局布局动作，且强调快捷键、菜单和窗口状态可以互相替代。Todo Agent 将这一点收敛为宽屏桌面专注布局：标题栏、快速命令和快捷键说明共用 `⌘/Ctrl + /`，收起只改变导航占用空间，不改变任务事实、当前路由或详情选择；窄桌面继续保留横向导航条，避免用户失去主要入口。

- **迁移收益**：处理长任务列表、表格和任务详情时减少固定导航占用，且不要求用户学习新的页面结构。
- **安全边界**：布局偏好只保存在本机，无法写入任务或飞书；快捷键在输入框、弹窗和窄桌面中不拦截。
- **证据**：[Things Mac keyboard shortcuts](https://culturedcode.com/things/support/articles/2785159/)。

#### 2.3.2 本轮桌面迁移：选中任务后就地新增

Things 将 Magic Plus 放在任务流中，让用户可以在当前上下文继续写，而不是先离开列表再回填位置；Todoist 的 Quick Add 也强调用一行输入快速捕获。Todo Agent 本轮把这一节奏收敛为 Today 的桌面 `Space`：只有“计划今天”分组中的选中任务显示可用提示，新增后通过现有私有顺序事务真实落在锚点后，并把创建与排序作为一组可反向回滚的连续动作。逾期和截止分组不做虚假插入，避免日期语义与视觉位置冲突；窄桌面、表格和飞书来源保持原交互。

- **迁移收益**：用户在整理今日执行顺序时不必把手移到页面顶部，也不需要先创建再拖回目标位置。
- **安全边界**：只写入本地任务；排序失败不撤销已成功创建的任务，撤销时先恢复顺序再撤销创建；输入框内保留原生空格和 Esc 语义。
- **证据**：[Things Mac keyboard shortcuts](https://culturedcode.com/things/support/articles/2785159/)、[Things Quick Entry](https://culturedcode.com/things/support/articles/2249437/)、[Todoist Quick Add](https://www.todoist.com/help/todoist/features/use-task-quick-add-in-todoist-va4Lhpzz)。

#### 2.3.3 本轮桌面迁移：全局查找直接落到可操作任务

Things 的 Quick Find 将搜索与导航合并，且支持键盘直接选择目标；这类体验的关键不是“找到一条结果”，而是打开后用户已经站在可以继续处理的位置。Todo Agent 本轮补齐全局查找的桌面闭环：空白状态可以用上下键浏览快捷搜索和最近搜索，结果活动项自动保持在结果面板内；选中任务后跨集合跳转，等待任务行稳定，必要时展开折叠分组，滚动到任务并恢复焦点。

- **迁移收益**：在长列表、完成记录或已折叠分组中，搜索结果不再只改变路由而把定位成本留给用户；回车后可以立即继续用 `E`、方向键或任务详情操作。
- **安全边界**：只改变当前桌面的视图、滚动位置和焦点，不修改任务数据；如果读取失败，保留原有错误与重试路径，不伪造定位成功。
- **证据**：[Things Mac keyboard shortcuts](https://culturedcode.com/things/support/articles/2785159/)、[Things Searching and Navigating with Quick Find](https://culturedcode.com/things/support/articles/2803584/)。

#### 2.3.4 本轮桌面迁移：暂存整理的焦点接力

Things 将 Inbox 定义为临时收集区：先把想法放进来，定期澄清，再移入列表或安排未来日期。Todo Agent 已有同样的“暂存”事实层，本轮把集中整理做成一条可连续执行的桌面仪式：打开弹窗即聚焦“今天”，跳过、完成或安排当前任务后焦点自动交给下一项；Tab 被限制在弹窗内，写入暂时忙碌且没有可用动作时焦点留在对话框；Escape、背景关闭和完成按钮会回到打开整理的触发按钮。

- **迁移收益**：整理两件以上暂存任务时，用户可以只用键盘完成“安排今天 / 明天、跳过、完成、打开详情”，不会在每项处理后重新寻找焦点；进度条也能被读屏正确理解为已处理与剩余数量。
- **安全边界**：跳过只改变本轮弹窗队列，不修改任务；今天/明天/完成仍复用普通任务控制器和原有错误处理；快速命令没有待整理任务时只进入空列表；焦点、Tab 边界和描述语义属于桌面 UI 状态，不进入任务、飞书、Agent 或导出数据。
- **证据**：[Things Today / Upcoming / Inbox](https://culturedcode.com/things/support/articles/4001304/)、[Things Mac keyboard shortcuts](https://culturedcode.com/things/support/articles/2785159/)。

#### 2.3.5 本轮桌面迁移：弹层焦点闭环与暂存单一语义

Things 的 Quick Find 与键盘工作流把“打开工具”视为当前工作流的暂时切换：用户可以用键盘进入、选择和退出，回到原来继续处理。Todo Agent 本轮将这一原则扩展到快速命令、全局查找和快捷键说明：三个弹层共用焦点陷阱，末项 Tab 回到首项、首项 Shift+Tab 回到末项；Escape 或关闭回到原触发点，真正执行命令或选择结果时则把焦点交给后续页面落点。

- **迁移收益**：键盘用户不会从搜索/命令窗口意外跳回背景列表；关闭后仍能在原工作位置继续，跨集合搜索的自动定位也不被恢复焦点覆盖。
- **安全边界**：焦点状态只属于桌面 UI；命令执行继续复用既有导航、确认和权限链，搜索选择只改变视图落点。暂存规则同时收敛为“开放、未删除、无项目/清单/计划日期/稍后日期/开始时间/截止时间”，服务层、离线回退和整理弹窗不再各自解释 Inbox。
- **证据**：[Things Searching and Navigating with Quick Find](https://culturedcode.com/things/support/articles/2803584/)、[Things Mac keyboard shortcuts](https://culturedcode.com/things/support/articles/2785159/)。

#### 2.3.6 本轮桌面迁移：跨集合跳转的稳定焦点交接

Things Quick Find 的连续体验还隐含一个重要条件：搜索结果离开弹层后，下一步键盘动作必须落在真实的目标对象上，而不能落在一次尚未完成的列表刷新中。Todo Agent 本轮针对全局查找的低概率竞态做了收口：集合或来源过滤发生变化时，控制器先把旧快照视为过渡态；目标任务连续多个渲染帧保持可见且真正成为 `activeElement` 后，才结束焦点请求；如果 Electron 主窗口重新获得焦点或恢复可见，会再次尝试交接。全局查找的并发读取也使用请求序列，只接受最新打开/重试产生的快照。

- **迁移收益**：长列表和跨集合定位后，用户可以立即继续使用 `E`、方向键或回车，不会遇到“详情已切换但焦点仍在页面空白处”的断点。
- **安全边界**：只改变刷新期间的 loading 门闩、滚动和键盘焦点；请求序列不会丢弃最新结果，旧结果不能覆盖新搜索，也不修改任务、项目、清单、会话或飞书数据。
- **验证**：目标场景独立重复 10 次、完整 67 条桌面 Electron 回归 65 项通过且 2 项按环境跳过；手机端布局与交互不在本轮范围内。
- **证据**：[Things Searching and Navigating with Quick Find](https://culturedcode.com/things/support/articles/2803584/)、[Things Today / Upcoming / Inbox](https://culturedcode.com/things/support/articles/4001304/)。

#### 2.3.7 本轮桌面迁移：批量编辑与行动项预览的连续键盘闭环

Things 的 Quick Find 与 Quick Entry 共同强调“用户决定下一步后立即继续输入或处理”。Todoist 的 Quick Add 也把快速捕获设计成短暂、可回收的工作层。本轮把同一原则扩展到两个更容易打断桌面节奏的确认弹层：批量编辑以及从会议/Agent 回复提取行动项。弹层打开后直接落到第一个有决策价值的字段，Tab 始终在当前弹层内循环；Escape、取消和遮罩关闭都回到触发按钮，而创建或预览中的异步状态禁用关闭动作，避免用户误以为操作已经回滚。

- **迁移收益**：用户可以从列表、时间线或 Agent 回复连续完成“打开 → 修改/勾选 → 确认或取消”，不必重新寻找原任务、会议按钮或 Agent 操作条；行动项草稿仍然逐项可编辑，确认前不会自动写入。
- **安全边界**：焦点交接只改变桌面 UI，不改变任务事实、日历事件、Agent 会话或飞书字段；弹层内的 Ctrl/⌘K、Ctrl/⌘N、全局查找和撤销等主窗口快捷键不会穿透到背景层，Escape 仍由当前最内层处理。
- **验证**：批量编辑组件、Agent 行动项组件、会议行动项组件、Markdown 操作按钮和时间线触发器均有单元覆盖；真实 Electron 场景验证首焦点、Tab 边界、弹层内快捷键隔离、Escape 关闭和触发器回焦，完整桌面回归为 68 条、66 项通过、2 项按环境跳过。
- **证据**：[Things Quick Entry](https://culturedcode.com/things/support/articles/2249437/)、[Things Mac keyboard shortcuts](https://culturedcode.com/things/support/articles/2785159/)、[Todoist Quick Add](https://www.todoist.com/help/todoist/features/use-task-quick-add-in-todoist-va4Lhpzz)。

#### 2.3.8 本轮桌面迁移：完整新建与权限确认的安全焦点路径

Things Quick Entry / Quick Find 的共同点是把“开始输入”和“回到原位置”做成同一条桌面路径；确认类交互则需要让用户清楚地知道当前决定仍在自己手里。Todo Agent 本轮把这条规则延伸到完整新建任务和 Agent 权限确认：新建任务打开即落到标题，Tab 在完整表单内循环，Escape 关闭后回到“新建”；权限确认默认把焦点放在“拒绝”，Escape 走同一拒绝路径，确认弹层不会把快捷键交给背景页面。

- **迁移收益**：完整任务编辑适合需要日期、提醒、标签和来源的场景，同时保持低摩擦连续输入；Agent 的批量或外部操作在键盘操作下仍先停在安全决策点，不会因焦点漂移而误执行。
- **安全边界**：新建任务沿用已有提交校验与本地/飞书写入边界；权限层只改变当前 UI 焦点，拒绝仍由原有 Agent 权限流程处理，忙碌或隐藏控件不会被纳入 Tab 顺序。
- **验证**：共享焦点 hook、研究卡、工作流模板和新建任务组件测试通过；真实 Electron 场景覆盖新建任务首焦点、Tab 回环、Escape 关闭和入口回焦，完整桌面回归为 69 条、67 项通过、2 项按环境跳过；手机端布局与交互不在本轮范围内。
- **证据**：[Things Quick Entry](https://culturedcode.com/things/support/articles/2249437/)、[Things Searching and Navigating with Quick Find](https://culturedcode.com/things/support/articles/2803584/)、[Todoist Quick Add](https://www.todoist.com/help/todoist/features/use-task-quick-add-in-todoist-va4Lhpzz)。
...
- **Todo Pet 落地（v2.17）**：快速捕获成功后在本机保留最多 12 条用户主动确认的最近捕获；快速录入窗支持一键回用和清空，宠物右键提供“最近捕获”入口。未确认的剪贴板、当前窗口、选中文本和拖入内容不写入历史，不上传、不写任务事件或飞书。
...
- **Todo Pet 落地（v2.16）**：吸收 Finch / Habitica 的“允许休假”边界，但不引入掉血或连续签到；设置和宠物右键菜单提供休假模式，保留任务、同步、专注和手动互动，暂停主动陪伴、工作时回应与自动日记，休假不会删除事实或产生缺席惩罚。
...
6. 宠物日记、共同经历卡、轻量装扮、共同收藏图鉴、共同完成印章、共同旅程章节和小窝；允许导出和删除。收藏图鉴只投影已有奖励库存，显示已解锁物品、数量和解锁提示；完成印章从真实已完成任务快照投影最近记录，今天的印章突出显示并可回到原任务，奖励对账未完成时明确显示同步中；旅程章节只投影项目任务的完成比例和下一步，不创建影子任务或第二套成长计数。任务详情现可显式把一项任务写入共同日记，保留原任务链接，不复制任务，也不把私人备注带入日记正文。
...
15. 依赖链解释卡：吸收 Asana / Linear / Taskwarrior 的阻塞链、项目健康和 urgency 可解释性；任务详情在已有依赖编辑下面展示前置、当前和后续任务，按关系深度排序，缺失或循环关系只提示不静默修复，点击节点回到原任务；高级入口可按需展开三列关系图，把“先做 → 当前 → 解锁”变成可点击节点，不把图谱变成默认工作台。
...
19. 受限任务自动化：吸收 Trello Butler、ClickUp Rules 和 Kanboard 自动动作的“事件 → 条件 → 动作”结构。设置允许最多 50 条本机规则，触发为新建、完成、用户手动应用、临近截止或按计划执行；条件编辑器支持共同条件（AND）和最多 5 组任一条件（OR），项目/清单优先显示本地实体名称；按计划规则支持每天或每周指定日期的本机时间，并用周期 checkpoint 防止重复；临近截止规则按 5 分钟至 7 天的本机提前窗口检查开放任务，过去截止、已完成和无截止日期的任务不触发，重复检查在动作已满足时自动成为 no-op。保存前可试运行预览当前匹配数量和前 5 项样例，明确预览只读、不写入任务或飞书；动作只更新重点标记、私有项目/清单/分组、稍后日期、标签和情境，复用普通更新与撤销；所有后台规则只处理开放任务。规则没有脚本、网络、任务创建或远端写回能力，执行串行且失败可解释。跨设备规则同步仍留在后续范围。
20. 统一本机查找：吸收 ClickUp 的“任务、文档、聊天、历史统一搜索”和 Notion / Joplin 的上下文检索习惯。`⌘/Ctrl+Shift+F` 或命令面板可在同一入口搜索任务标题、备注、私人备注、标签、情境、评论、研究卡、附件/链接元数据、自定义字段、已导入本地日历事件，以及项目、清单和本机会话；标题精确匹配优先，多个关键词必须同时满足，结果显示来源、状态和可读片段。选择任务回到 All 并定位原任务，选择日历事件回到对应日期的时间线，选择会话直达 Agent 历史，项目/清单进入对应工作区。索引只来自本机已保存快照，不读取附件正文、不上传、不改任务、日历或飞书；搜索失败可重试，空查询不展示隐式内容。
21. 低摩擦搜索回用：延伸 Todoist、ClickUp 和 Raycast 的最近查询体验。全局查找只在用户真正打开一个结果后记住最近 8 条查询，空查询页提供可点击回用和一键清空；查询文本做长度上限、空白归一化和大小写去重，历史只存在本机浏览器存储，不进入 Agent、飞书、任务导出或宠物记忆。存储损坏或配额不足时搜索仍可正常使用。
22. 命名搜索快捷项：吸收 Todoist Filters、OmniFocus Perspectives 和 Vikunja Saved Filters 的“把常用问题变成入口”模式。在全局查找中可将当前关键词保存为本机命名快捷项，空查询页直接打开、复用、单项删除或全部清空；同一查询再次保存时更新名称而不制造重复项，最多保留 12 项。快捷项只保存查询文本和显示名称，不复制任务、不改变保存视图、不进入 Agent/飞书/导出或宠物记忆；浏览器存储损坏、配额不足和异常输入都 fail-closed，不影响普通搜索。
23. 多日历来源控制：吸收 Morgen / Any.do / Structured 的多日历聚合与“在同一时间线中按来源聚焦”习惯。时间线现在把本地 `.ics` 导入事件按 `sourceName` 聚合，用户可以独立显示/隐藏工作与个人等来源，并为每个来源使用稳定默认颜色或手工调整颜色；颜色和隐藏偏好只保存在本机且不改变原始事件。若用户提供日历写入回调，还可只清除一个来源并通过一次撤销恢复，避免为了暂时整理视图而误删其它日程。筛选和着色只影响时间线与周负载投影，Today 规划继续使用完整事件事实来保护容量，不写回外部日历、飞书或任务共享字段。

### P2：扩展内容和生态

1. 声明式 JSON 动作包已支持安装、启用、更新和卸载；设置新增可视化动作包编辑器，可加载当前包、组合内置待机动作并安装启用，编辑器与 JSON 入口共用同一字段白名单和校验器。动作包现在还支持 8–60 秒动作冷却与 1–5 动作出现频率偏好，待机调度按确定性权重选择并避免过密重复，旧包缺字段时沿用原有节奏。工作流模板已支持内置流程和安全 JSON 导入，模板只能描述任务字段和相对日期，不携带脚本、网络请求或任意文件操作。季节小事件已落地为可关闭的本地日期装饰与温和提示，只改变宠物外观和气泡，不影响任务、成长或同步；颜色主题包已扩展为本机四色编辑器加受限本地 PNG/JPEG/WebP 背景，仍不接受 SVG、网络地址、图片路径、脚本或任意 CSS；第三方连接器和网络素材市场仍列为后续扩展。
2. 多宠物协作或不同人格的 Agent：临时“小队一起开工”、宠物人格联动、当前伙伴领队/角色简报、领队优先的逐步准备节奏和最多 3 项的小队路线预览已落地，仍由一个任务事实层管理；更丰富的跨宠物工作流、真实 Agent 角色编排和跨任务协作后续再做。
3. 移动端 Widget、Live Activity、Android 悬浮层和跨设备状态。
4. 可选的共享清单、家庭任务和低压力协作。
5. 季节事件（已落地 v1.40 的轻量日期装饰）；成长首页的“今日明信片”已把真实完成、专注、今日待办和天气组合成一次性陪伴投影；更丰富的家园装饰和社区内容仍后续，不建立排行榜压力。
6. 低压力休假（已落地 v2.16）：用户可以暂时暂停宠物主动陪伴、工作时回应和自动日记，任务、同步、专注、提醒与手动互动继续可用；不删除任务、不清空成长、不维护连续签到，恢复后直接回到原状态。
...
- **Todo Pet 落地（v2.15）**：设置提供默认关闭的“工作时回应”；主进程只读取系统空闲秒数，按 4 秒轮询、14 秒节流广播 `typing` / `reading` 姿态，宠物显示内置键盘/阅读动作。会议、隐私和隐藏状态不广播，绝不读取具体按键、指针、窗口标题或输入内容。
...
| 2026-08-21 | v1.92 | 延伸 Shimeji-ee / Weyrdlets 的可扩展动作生态：设置新增可视化动作包编辑器，用户可以从 20 个内置待机动作中组合、载入、清空并安装启用一个安全声明式动作包；仍不接受脚本、网络、文件或外部素材，带素材主题包与第三方连接器继续后置 |
| 2026-08-21 | v1.93 | 延伸 Study Bunny / Tamagotchi 的收藏反馈：小窝新增共同收藏图鉴，按 PetService 已有库存投影装扮、摆件、动作和纪念品，显示解锁进度、数量与来源提示；不新增任务副本、不改变奖励规则、不写回飞书 |
| 2026-08-21 | v1.94 | 延伸 Tamagotchi / Linear 的“共同旅程”反馈：小窝成长页新增项目章节卡，按现有项目任务投影完成比例、开放任务和可执行下一步；支持打开项目或任务，不新增计数、不改任务状态、不写回飞书 |
| 2026-08-21 | v1.95 | 延伸 Study Bunny / Tamagotchi 的“完成反馈”和 Desktop Pet 的安全调皮动作：小窝成长页新增共同完成印章卡，从真实已完成任务投影最近印章与今日数量，点击可回到原任务；奖励未对账时显示同步中，不创建任务副本、不重复计数、不改变任务或飞书字段 |
| 2026-08-21 | v1.96 | 延伸 Finch / Spirit City / Weyrdlets 的小窝个性化：房间云灯、小植物和任务书架新增安全的本地位置与大小编辑，用户可用方向键微调、滑杆缩放并恢复默认；旧档案自动投影平衡布局，未知/越界值由白名单和边界收敛，不进入任务、Agent 或飞书同步 |
| 2026-08-21 | v1.97 | 延伸 Spirit City 的环境层：Todo Pet 小窝新增明亮日光、柔和灯光和月光夜色三档纯本地视觉氛围；不自动播放声音、不读取天气或系统时间，不改变任务、提醒、成长或飞书同步，旧档案默认明亮日光 |
| 2026-08-21 | v1.98 | 延伸 Desktop Pet / Shimeji 的直接操控：小窝摆件支持在房间舞台按住拖动，坐标转换为受边界限制的本地百分比，拖动结束只提交一次外观更新；方向键和大小滑杆仍作为键盘/精细调整入口，不产生任务或飞书写入 |
| 2026-08-21 | v1.99 | 延伸 Finch / OpenDesktopPet 的人格陪伴：设置默认让 Agent 跟随 Todo Pet 六种陪伴性格，下一轮任务对话和晨间简报使用对应表达规则；用户可关闭联动让 Agent 保持独立 Persona，性格标签不携带宠物任务/记忆/成长，也不改变权限、模型数据范围或飞书同步 |
| 2026-08-21 | v2.00 | 延伸 Desktop Pet / Spirit City 的主题生态：小窝新增安全的本地颜色主题包编辑器，支持最多 12 个包、应用/移除、可读 JSON 导出与导入预览和四个六位十六进制颜色；安装严格拒绝脚本、URL、图片路径、任意 CSS 和未知字段，主题只改本机房间视觉，不进入宠物档案、任务、Agent 或飞书同步；外部素材包和第三方连接器继续后置 |
| 2026-08-21 | v2.01 | 延伸 Spirit City / Desktop Pet 的季节陪伴：季节小事件不再只有悬浮标签，小窝增加按本地日期切换的内置花朵、微风、落叶和雪花装饰层；装饰只使用固定字符与 CSS，尊重关闭季节事件和减少动态设置，不保存状态、不增加奖励、不改变任务或飞书同步 |
| 2026-08-21 | v2.02 | 延伸 Desktop Pet / Weyrdlets 的多伙伴方向：小窝伙伴从静态陪伴头像升级为可点击互动，按伙伴种类和陪伴性格给出短暂动作与本地问候；不复制任务、不建立第二事实层、不进入 Agent 或飞书同步，键盘与减少动态设置均有安全回退 |
| 2026-08-21 | v2.03 | 将主题包从四色配置扩展为受限本地素材：编辑器支持选择 512 KB 以内的 PNG/JPEG/WebP 背景并随 JSON 主题包迁移，导入仍先预览；不接受 SVG、网络 URL、图片路径、脚本或任意 CSS，第三方连接器和网络主题市场继续后置 |
| 2026-08-21 | v2.04 | 延伸 Finch / Weyrdlets / Desktop Pet 的多伙伴协作：小窝在存在开放任务时提供临时“小队一起开工”，按伙伴种类映射侦察、稳节奏、整理和守护角色，准备动画后可开始同一原任务专注或打开任务；分工只属于当前陪伴层，不复制任务、不写入任务字段、不改变飞书同步 |
| 2026-08-21 | v2.05 | 延伸 Desktop Pet / PawPal 的桌面状态提示：Todo Pet 在非仅宠物的紧凑形态增加可点击天气胶囊，复用用户手工配置城市的现有天气缓存，显示温度、状况、降水、缓存和严重天气提示，点击进入既有天气卡片；隐私模式隐藏城市，不读取定位、不新增网络权限、不改变任务或飞书同步 |
| 2026-08-21 | v2.06 | 延伸 TickTick Widgets / Routine Menu Bar Widget 的低摩擦入口：macOS 菜单栏与 Windows 系统托盘增加只读“今日待办”预览，显示最多 3 项真实未完成任务和总数，点击回到同一 Today 视图；隐私模式脱敏标题，任务读取失败只降级菜单，不建立第二任务事实层、不新增写入权限 |
| 2026-08-21 | v2.07 | 延伸 Finch / Weyrdlets 的陪伴群体与多角色方向：小队一起开工允许用户从当前伙伴中选择临时领队，每位伙伴展示固定角色与一句协作提示，领队在准备/就位阶段承担第一步叙事；选择只存在于当前 UI，不保存、不复制任务、不调用额外 Agent 工具、不改变任务或飞书状态 |
| 2026-08-21 | v2.08 | 延伸 Finch / Weyrdlets 的陪伴群体与准备仪式：小队开工卡新增可展开的协作简报，领队优先展示第一步，其余伙伴按固定角色逐步点亮，准备结束后明确显示“已就位”；这是本地陪伴层的解释与节奏反馈，不模拟 Agent 执行、不创建子任务、不调用额外工具、不改变任务或飞书状态 |
| 2026-08-21 | v2.09 | 延伸 Finch / Weyrdlets 与 TickTick 的陪伴工作流：小队开工卡新增最多 3 项的当前路线预览，按真实开放任务和预计时长给出候选与总时长，点击即可切换当前开工目标；路线只改变 UI 选择，不批量修改、不自动串联、不创建任务副本、不改变任务或飞书状态 |
| 2026-08-21 | v2.10 | 延伸 Finch / Spirit City 的轻量首页陪伴：成长首页新增“今日明信片”，把真实的今日完成、专注分钟、今日待办和天气快照组成一张按事实变换语气的非持久化卡片，并可回到 Today；不创建任务、不增加第二套成长计数、不形成记忆、不写入飞书 |
| 2026-08-21 | v2.11 | 延伸 PawPal / Spirit City 的环境感知与低打扰提醒：天气服务从单日当前值扩展为同一手工城市缓存下最多 3 天的结构化预报，小窝天气卡展示今天/明天/后天的天气、低高温、降水和严重标记；预报缺失或过期只降级为可用缓存，不读取定位、不调用模型、不创建任务、不改变飞书同步 |
| 2026-08-21 | v2.12 | 延伸 Tamagotchi / PawPal 的环境陪伴：宠物本体根据结构化天气代码显示固定雨伞、雪花或雷云闪电反应，晴朗/未知天气保持干净；反应不自动换装、不增加提醒、不读取定位、不改变任务、成长或飞书同步 |
| 2026-08-21 | v2.13 | 延伸 Shimeji-ee / Weyrdlets 的动作编排实践：自定义动作包增加 8–60 秒冷却和 1–5 频率偏好，待机调度按确定性权重选择并尊重最小间隔；旧 JSON 包兼容，仍只允许内置动作，不执行脚本、网络或文件操作 |
| 2026-08-21 | v2.14 | 延伸 Desktop Pet / PawPal 的桌面层能力：新增默认关闭的鼠标穿透，宠物可保持置顶可见而把点击交给后方窗口；设置与系统托盘都能恢复，迁移兼容旧配置，不改变任务、同步、Agent、专注或权限 |
| 2026-08-21 | v2.15 | 延伸 Cat Fidget / BongoCat 的输入映射：新增默认关闭的工作时回应，以系统空闲秒数低频触发敲键盘/阅读姿态；不读取具体输入、不上传，会议/隐私/隐藏时停用，旧配置和导入默认关闭 |
| 2026-08-21 | v2.16 | 延伸 Finch / Habitica 的低压力休假边界：新增休假模式，暂停宠物主动陪伴、工作时回应和自动日记，但保留任务、同步、专注、提醒和手动互动；不删除事实、不维护连续签到、不产生缺席惩罚，旧配置和导入默认关闭 |
| 2026-08-21 | v2.17 | 延伸 Akiflow / Things Inbox 的快速回用：快速捕获成功后本机保留最多 12 条确认记录，快速录入窗支持回用与清空，宠物右键提供最近捕获入口；未确认上下文不写历史、不上传、不进入任务事件或飞书 |
| 2026-08-21 | v2.18 | 延伸 Taskade / ClickUp 的 Agent 能力分层与工具注册表：权限中心新增任务读写、飞书同步、网页研究、文件与终端、剪贴板与屏幕五个独立开关；工具注册表按能力族过滤模型可见工具，并在准备/执行前重检，用户撤权可立即收敛；飞书能力关闭时读取与写回均 fail-closed，不把远端操作降级为本地任务；能力设置与模型数据范围、风险审批、宠物状态和飞书 payload 分离，旧设置与备份缺字段时默认全开 |
| 2026-08-21 | v2.19 | 延伸 Notion AI / Taskade 的 Agent 可解释执行：主 Agent 对话把可信运行事件投影为短期“执行过程”卡，显示工具拟定、等待确认、执行中、已完成、拒绝、失败、停止和复用结果，并可展开受权限引擎生成的影响预览；活动不进入聊天历史、不上传、不替代审计，权限拒绝也明确收敛为终态，避免把模型最后一句话误认为执行成功 |
| 2026-08-21 | v2.20 | 延伸 Routine / Akiflow / Superlist 的上下文捕获回用：快速录入中的剪贴板、当前窗口、选中文本和文本/链接拖入预览，只有在用户明确点击“保存到最近上下文”后才保留最多 12 条本机记录；支持回用与清空，未保存预览、文件内容、窗口持续监听、Agent/飞书同步和数据导出均不进入历史 |
| 2026-08-21 | v2.21 | 将上下文捕获回用延伸到 Todo Pet 小窗：宠物收到的文本/链接拖入和选中文本预览也提供显式保存入口，并沿用同一份本机最多 12 条历史；宠物小窗只增加保存反馈，不自动上传、读取文件正文、发送给 Agent 或写入飞书 |
| 2026-08-21 | v2.22 | 延伸 Superlist Talk / OpenDesktopPet 的语音陪伴方向：主 Agent 与 Todo Pet 小窗的 Markdown 回复增加显式“朗读/停止朗读”，在本机使用系统语音合成，代码块仅提示省略；默认不自动播放、不保存音频、不上传文本，系统不支持时安全隐藏为不可用状态 |
| 2026-08-21 | v2.23 | 将 Superlist Talk / OpenDesktopPet 的主动语音陪伴延伸到晨间简报、当前任务气泡和专注气泡：用户可在看到摘要或宠物状态时手动朗读，任务轮播/摘要刷新会先停止旧文本；隐私模式只朗读“私人任务”提示，默认不自动播放、不保存音频、不上传，所有入口共用一个可停止的本机语音通道 |
| 2026-08-21 | v2.24 | 延伸 Routine / Akiflow / Superlist 的上下文捕获回用与 Taskade 的持久项目上下文：Agent 完成后的研究回复新增显式“保存到最近上下文”，使用独立 `agent-reply` 类型并复用本机最多 12 条/单条 2,000 字的历史；流式回复不可保存、不会自动记录或发送给模型，清空与回用仍沿用快速捕获入口，不进入飞书、任务事件或云端 |
| 2026-08-21 | v2.25 | 延伸 Joplin / Anytype 的可迁移与可读输出：Agent 完成回复新增显式“复制 Markdown”，把原始 Markdown 写入系统剪贴板，便于带到文档、Issue 或其他编辑器；流式回复和不支持剪贴板的环境保持禁用，不自动复制、不上传、不写入任务、飞书或上下文历史 |
| 2026-08-21 | v2.26 | 延伸 Taskade / Notion 的短期上下文复用：最近上下文历史现在可直接从主 Agent 和 Todo Pet 聊聊入口回用，点击只把原文追加到输入框并保留 Markdown；不会自动发送、不会重新保存同一条、不进入模型历史范围以外的上下文，清空仍是本机统一操作 |
| 2026-08-21 | v2.27 | 延伸 Superlist / Joplin / Notion 的“上下文长出行动项”：主 Agent 完成回复提供显式“提取行动项”，仅从明确行动项/待办/下一步标题、清单或动作句生成最多 8 个本地草稿；用户可编辑、逐项取消并确认后创建本地任务，研究内容不自动执行、不直接写飞书，沿用网页研究边界提示 |
| 2026-08-21 | v2.28 | 延伸 Joplin / Notion / Taskade 的“研究回到任务上下文”：主 Agent 在任务已选中时新增“保存研究卡”，先以确认面板编辑标题、来源链接、Markdown 摘要和行动项，再写入当前任务的本地私人研究卡；最多 20 张/卡片 20 条行动项，来源只接受无凭据的 http/https，流式回复不提供入口，研究卡不写回飞书、不自动执行网页指令 |
| 2026-08-21 | v2.29 | 延伸 Joplin / Notion 的可读研究上下文：任务详情展开研究卡时将 Markdown 摘要渲染为标题、列表、表格、引用和代码块，链接仍通过桌面安全出口打开；原始 HTML、远程图片和不安全协议始终降级为文本，渲染只读、不执行研究内容、不写回飞书 |
| 2026-08-21 | v2.30 | 延伸 Joplin / Superlist 的“研究长出行动”：研究卡每条行动项增加显式“创建任务”入口，创建前不会自动执行；生成的任务固定为本地来源，保留研究卡/原任务/来源链接 provenance，沿用普通任务控制器和通知反馈，绝不写回飞书；同一张卡在当前打开会话中已创建的行动项会禁用重复入口 |
| 2026-08-21 | v2.31 | 延伸 Apple Reminders / Trello / Notion 的模板回用：任务详情新增“保存为工作流模板”，可编辑模板名称和说明，将备注、标签、优先级与预计时长转换为一个受限本地步骤；完成状态、具体日期、附件、同步身份和飞书来源不会复制，后续仍先预览再创建，不执行脚本或写回飞书 |
| 2026-08-21 | v2.32 | 延伸 Trello Checklist / Notion Database Template 的结构复用：保存任务模板时可选择带入最多 11 个子任务；模板增加受限的父步骤引用，使用本地模板时按创建顺序恢复父子关系，使用飞书来源时不把本地任务 ID 传入远端；模板预览明确步骤数量，仍不复制完成状态、附件、日期或同步身份 |
| 2026-08-21 | v2.33 | 延伸 Todoist / Notion / ClickUp 的批量编辑：任务列表选中多项后可统一修改优先级、项目、清单和标签（替换/追加/移除）；先展示精确目标和属性差异，主进程原子校验基线并生成单一撤销操作；这些本地上下文字段不写回飞书，已删除任务不能参与编辑 |
| 2026-08-21 | v2.34 | 延伸 Todoist / Microsoft To Do 的重复任务处理：本地循环任务详情提供“跳过本次”，在同一任务上把日期、开始/截止时间、时间块和提醒平移到下一次循环；不创建重复任务，支持单次撤销；飞书循环继续由飞书负责，专注中的任务和已到最后一次的循环 fail-closed |
...
| 2026-08-21 | v2.41 | 吸收 Microsoft To Do / Any.do 的低压力提醒解释：每条提醒由本地投影携带“你设置的提醒 / 截止时间 / 晨间简报 / 合并错过提醒”等可信原因，处理面板提供可折叠的“为什么现在提醒”；解释不由模型生成，不把外部任务正文当成系统事实，也不改变任务或飞书同步 |
| 2026-08-21 | v2.42 | 吸收 Trello Butler、ClickUp Rules 与 Kanboard 自动动作：设置新增最多 50 条本地任务自动化，支持新建/完成触发、来源/项目/清单/分组/标签/情境条件和重点/私有组织字段动作；规则只复用普通任务更新与撤销，不执行脚本、不联网、不创建任务、不写回飞书，失败保持可解释且不循环重试 |
| 2026-08-21 | v2.43 | 延伸 Trello Butler 的卡片按钮：自动化新增“手动应用时”触发器，任务详情的更多操作列出启用的手动规则；条件不匹配或已经是目标状态时按钮禁用并解释原因，点击后只更新原任务私有字段并提供单次撤销。手动规则不会被后台事件触发，不执行脚本、不联网、不创建任务、不写回飞书 |
| 2026-08-21 | v2.44 | 延伸 Trello Butler 的批量按钮与 ClickUp Rules 的批量动作：批量选择工具栏展示手动规则、符合条件的可应用数量和动作预览，确认后由主进程一次事务应用到所有目标并生成单一撤销记录；规则、条件或任一任务基线发生变化时整批拒绝，不部分写入，不执行脚本、不联网、不创建任务、不写回飞书 |
| 2026-08-21 | v2.45 | 补齐受限自动化的图形化条件组合：设置页使用来源下拉、项目/清单实体选择和分组/标签/情境建议输入，所有已填写条件按 AND 语义同时满足，并在已保存规则中显示完整摘要；未知历史 ID 可安全保留，仍不提供定时器、脚本、网络请求或跨设备同步 |
| 2026-08-21 | v2.46 | 吸收 Trello Butler / ClickUp Rules 的时间触发器：自动化支持每天或每周指定日期的本机时间，主进程每 30 秒检查，错过设定分钟会在当天补执行一次，同一周期用 checkpoint 去重；定时规则只处理开放任务并复用本地条件、私有动作和撤销，不执行脚本、不联网、不创建任务、不写回飞书 |
| 2026-08-21 | v2.47 | 延伸 Kanboard 的“临近截止自动动作”：自动化新增 5 分钟至 7 天的本机提前窗口触发，主进程每 30 秒检查进入窗口的开放任务；重复检查只在私有动作产生实际变化时写入，过去截止、已完成和无截止日期的任务不触发，截止时间与飞书共享字段永不改写；仍不执行脚本、不联网、不创建任务、不写回飞书 |
| 2026-08-21 | v2.48 | 延伸 Trello Butler / ClickUp Rules 的条件组合：任务自动化条件新增最多 5 组可折叠的“任一条件”分支，形成“共同条件 AND（任一分支 OR）”；分支字段仍限来源、项目、清单、分组、标签和情境，空分支、嵌套分支和未知字段 fail-closed，已保存规则摘要会显示 OR 结构，不改变任务、飞书或 Agent 权限边界 |
| 2026-08-21 | v2.49 | 延伸 Trello Butler / ClickUp Rules 的可验证体验：设置页增加自动化“试运行预览”，按当前本机任务快照展示匹配数量、前 5 项样例和触发器限制；预览只读，不创建任务、不写入任务历史、不写飞书或同步队列，规则仍需用户明确保存后才启用 |
| 2026-08-21 | v2.50 | 延伸 ClickUp 的统一搜索与 Notion / Joplin 的上下文检索：新增全局查找入口（`⌘/Ctrl+Shift+F` 与命令面板），跨任务元数据、项目、清单和本机会话返回确定性排序结果；精确标题优先、多个关键词同时满足，选择结果可回到任务/Agent/项目/清单。索引只读本机快照，不读取附件正文、不上传、不写任务或飞书 |
| 2026-08-21 | v2.51 | 延伸 Todoist、ClickUp 与 Raycast 的搜索回用：全局查找在用户打开结果后保留最近 8 条本机查询，空查询页提供点击回用和一键清空；查询只做长度/空白/大小写归一化，不进入 Agent、飞书、任务导出或宠物记忆，存储异常时保持可用 |
| 2026-08-21 | v2.52 | 延伸 Todoist Filters、OmniFocus Perspectives 与 Vikunja Saved Filters：全局查找可把当前查询命名保存为最多 12 条本机快捷项，支持空查询页一键复用、单项删除和全部清空；同一查询更新名称而不重复，快捷项只保存名称/查询文本，不改任务、保存视图、Agent、飞书或导出，异常存储保持普通搜索可用 |
| 2026-08-21 | v2.53 | 延伸 Morgen / Any.do / Structured 的多日历聚合体验：时间线按本地 `.ics` 的来源独立显示/隐藏，偏好保存在本机；在允许写入日历事件时可单独清除某个来源并用一次撤销恢复。来源筛选只改变时间线与周负载显示，规划器仍使用完整日历事实保护容量，不写回外部日历、飞书或任务 |
| 2026-08-21 | v2.54 | 延伸 Morgen / Structured 的多日历色彩辨识：每个本地 `.ics` 来源获得稳定的默认颜色，时间线事件卡和来源筛选使用同一色标，用户可手工调整并恢复默认；颜色偏好仅保存在本机，不改变事件、容量规划、外部日历或飞书 |
| 2026-08-21 | v2.55 | 延伸 ClickUp / Notion / Joplin 的统一本机检索：全局查找新增本地日历事件结果，标题、备注、来源和日期均可匹配；打开结果后主窗口直达对应日期的时间线，仍只读本机缓存，不写日历、任务或飞书 |
| 2026-08-21 | v2.56 | 延伸 Asana / Linear / Taskwarrior 与 Anytype 的高级关系视图：任务详情的依赖链新增按需展开的三列关系图，节点可回到原任务，缺失和循环依赖保持可见；图谱只读本地快照，不修改任务、计划或飞书 |
| 2026-08-21 | v2.57 | 修复 OpenAI-compatible 模型测试容易误用旧凭据的问题：设置中输入新 API Key 后点击测试会先安全替换本机凭据，再发起连接请求；密钥不进入日志、提示词、导出或研究数据 |
| 2026-08-21 | v2.58 | 吸收 Trello / Kanboard 的低摩擦排序：Today 默认顺序提供可访问的六点拖动把手，拖到另一项开放任务上按插入位置更新私人排序；带筛选、已完成和批量模式隐藏拖动入口，键盘上移/下移和原子撤销保持不变，拖放不修改飞书共享字段 |
| 2026-08-21 | v2.59 | 延伸飞书 / Todoist / Vikunja 的可信同步反馈：同步页在手动同步后展示最近一次全量/增量运行摘要，区分本次上传、拉取、删除、冲突数量与权限/网络/重试原因；明确数量只是本次变化，不把变化数误报为远端总任务数，私人 Today、排序和专注字段仍不写回 |
| 2026-08-21 | v2.60 | 吸收 Vikunja / Focalboard 的 Table View：任务集合新增列表 / 表格切换，表格按同一任务快照展示状态、标题、日期、优先级、项目/清单和来源，并保留完成、重点标记、批量选择、详情与 Agent 入口；视图偏好只保存在本机，Today 仅列表视图允许私人顺序拖动，不复制任务、不写回飞书 |
| 2026-08-21 | v2.61 | 修复 OpenAI-compatible Chat Completions 兼容性：部分网关（含当前 gpt-5.6-sol 端点）在没有工具调用时返回 `message.tool_calls: null`，网关现接受省略、`null` 与工具调用数组，避免有效回答被误报为连接失败；不改变工具白名单、权限或任务事实 |
| 2026-08-21 | v2.62 | 吸收 Vikunja / ClickUp / Asana 的路线规划视图：时间线新增只读 14 日甘特投影，按项目分组展示任务的开始/截止/Today 计划跨度、预计进度和前置依赖阻塞；支持项目筛选、前后窗口导航、点击回任务，未安排日期的任务保留在托盘；不复制任务、不自动改期、不写回飞书 |
| 2026-08-21 | v2.63 | 延伸 ClickUp / Asana / Linear 的路线风险表达：甘特视图从现有项目依赖图和日期跨度确定性计算至少两项组成的最长依赖链，标出关键路线任务并显示数量；独立任务、缺失依赖和循环依赖不被误标或静默修复，结果只读，不改变任务、计划或飞书 |
| 2026-08-21 | v2.64 | 延伸 ClickUp / Asana / Linear 的依赖导航：甘特视图新增只读关键路线链摘要，按项目展示“前置 → 中段 → 终点”，每个节点可回到原任务；链摘要复用同一依赖图和任务 ID，不创建路线实体、不改变任务或飞书 |
| 2026-08-21 | v2.65 | 延伸 OpenAI-compatible 网关的连接可诊断性：发送前清理 Endpoint、模型名与 Bearer Key 的首尾空白，避免剪贴板换行造成无意义的 401 或模型名不匹配；不改变凭据存储、权限或请求范围 |
| 2026-08-21 | v2.66 | 延伸 Vikunja / ClickUp / Asana 的路线规划体验：甘特视图支持 2 周、4 周和 12 周时间窗，窗口以周一为起点并按当前尺度导航；本机记忆用户选择，长窗口保持可横向阅读；时间窗只改变只读投影范围，不复制任务、不改日期或时间块、不写回飞书 |
| 2026-08-31 | v2.91 | 吸收 Things 3 / Todoist 的低摩擦捕获：Today、暂存和全部任务的桌面列表提供一行式快速添加，回车创建本地任务，Today 自动带当天私人计划，暂存保持未安排；成功后继续聚焦输入框，便于连续记录。搜索、筛选、飞书来源和批量选择时隐藏入口，避免当前投影视图与新任务落点不一致；完整属性仍通过原有新建表单编辑，不新增任务事实层或外部写入 |
| 2026-08-31 | v2.92 | 吸收 Things 3 / Linear 的低摩擦详情处理：宽屏任务详情支持收起与恢复，收起后保留当前任务上下文并释放完整列表工作区；恢复后回到同一任务，切换另一项任务时自动恢复详情；紧凑桌面仍沿用覆盖式详情，不新增移动端布局 |
| 2026-08-31 | v2.93 | 修复 Agent 欢迎语在任务快照加载前固化为旧数量的问题：加载中显示读取状态，快照到达后同步当前视图数量，并迁移本机旧欢迎语；只更新自动生成的首条提示，不改变对话历史、任务事实、权限或模型数据范围 |
| 2026-08-31 | v2.94 | 吸收 Things / Todoist 的桌面键盘导航：任务列表与表格支持 ↑↓、J/K 连续浏览，焦点跟随并滚动到当前任务，无选择时从首项或末项开始且不循环；输入框、筛选弹层、批量模式和排序忙碌状态保持隔离，紧凑桌面不增加额外控件 |
| 2026-08-31 | v2.95 | 延伸 Things / Todoist 的桌面键盘节奏：任务列表与表格点选后可按 E 完成/恢复，复用原有同步提示、撤销和飞书权限边界；按键提示使用可识别的按键样式，异步更新期间防重复提交，输入框、筛选弹层、批量模式和排序忙碌状态保持隔离 |
| 2026-08-31 | v2.96 | 延伸桌面效率产品的连续处理节奏：快捷完成/恢复成功后自动把选择和焦点交给相邻任务，末项回退上一项；没有相邻项、失败或不可切换时保留当前上下文，不改变任务权限与同步语义 |
| 2026-08-31 | v2.97 | 延伸 Linear 的高亮与批量选择节奏：任务列表和表格点选后按 X 进入批量模式并切换当前选择，批量模式内继续用 ↑↓ / J K 浏览并扩展选择；保留既有预览确认、一次撤销、输入框隔离和飞书权限边界 |
| 2026-08-31 | v2.98 | 延伸桌面效率产品的连续处理节奏：快捷完成/恢复后只有在真实 activeElement 落到相邻任务时才结束焦点交接，后台窗口重新获得焦点时会补偿；任务筛选“清除”统一恢复重点标记、日期与排序，避免保存视图或临时筛选留下隐性状态 |
| 2026-08-31 | v2.99 | 吸收 Todoist 的 Shift 点击范围选择与 Esc 取消语义：普通任务列表以当前任务为锚点进入连续多选，列表/表格批量模式可扩展范围；筛选、批量编辑、批量预览、批量选择和宽屏任务详情按 Esc 分层退出并恢复触发焦点，隐藏分组、输入法组合和忙碌状态保持安全边界 |
| 2026-08-31 | v2.100 | 延伸 Todoist 的多选工具栏焦点捷径：列表/表格进入批量模式后按 `,` 把焦点交给第一个可用批量操作，保留输入框、排序忙碌和批量执行期间的按键隔离，降低任务列表与批量工具栏之间的鼠标往返 |
| 2026-08-31 | v2.101 | 吸收 Todoist 的 `?` 快捷键说明入口与 Linear 的可搜索帮助窗口：桌面标题栏提供键盘入口，按 `?` 可从非输入区域打开同一面板；按全局操作与任务列表分组，支持关键词筛选、平台修饰键、快速命令跳转、Esc 关闭和触发焦点回收；输入框、其他弹层和输入法组合保持隔离 |
| 2026-08-31 | v2.102 | 延伸 Todoist 的多选交互：列表与表格支持按住 ⌘/Ctrl 点选不连续任务，首次点选保留当前任务作为第一项，后续点击只切换目标，Shift 范围选择仍优先；输入框、隐藏任务、批量执行和权限边界保持隔离 |
| 2026-08-31 | v2.103 | 吸收 Linear 的全局撤销节奏：主窗口支持 ⌘/Ctrl+Z 撤销最新未撤销的本地任务操作，命令面板和可搜索快捷键说明提供同一入口；操作摘要只跨 IPC 暴露 ID、类型和时间，不携带任务快照，撤销后刷新列表/时间线并回到上一条可撤销操作；输入框、弹窗、重复按键、同步冲突和后续修改保持安全边界 |
| 2026-08-31 | v2.104 | 延伸 Linear 的 Undo / Redo 节奏：主窗口支持 ⌘/Ctrl+Shift+Z 重做最近一次安全撤销的本地任务变更，命令面板和可搜索快捷键说明提供同一入口；以持久化撤销顺序保证同一时间戳下仍按 LIFO 回放，新任务变更会使旧重做分支失效；重做前由主进程重新校验快照、飞书同步影响和冲突，输入框、弹窗和权限边界保持隔离 |
| 2026-08-31 | v2.111 | 吸收 Things Quick Find / 键盘工作流的连续处理节奏：快速命令、全局查找和快捷键说明共用 Tab 焦点边界，关闭回到原触发点，执行命令或选择搜索结果把焦点交给后续落点；同时将服务层、离线回退与暂存整理收敛到同一“开放且完全未安排”的 Inbox 规则，避免同一任务在不同刷新路径中出现不同归属 |
| 2026-09-01 | v2.112 | 修复桌面全局查找跳转的焦点竞态：集合切换的刷新完成前不提前确认焦点，目标行需连续稳定获得焦点才结束交接，主窗口重新激活或可见时自动补偿；全局查找读取只接受最新请求，关闭/重复打开不会被旧请求回写。相关单元、目标 Electron 场景重复 10 次及完整 67 条桌面回归通过（65 项通过、2 项按环境跳过） |
| 2026-09-01 | v2.113 | 延伸 Things / Todoist 的连续处理节奏：批量编辑、会议行动项和 Agent 行动项预览接入共享 Tab 焦点边界与首字段聚焦；Escape、取消和遮罩关闭回收至真实触发按钮，保存中保持弹层；弹层内 Ctrl/⌘ 快捷键不穿透到主窗口。完成相关单元、批量编辑与时间线 Electron 场景及完整 68 条桌面回归（66 项通过、2 项按环境跳过） |
| 2026-09-01 | v2.114 | 延伸 Things / Todoist 的桌面低摩擦输入与安全确认：新建任务完整表单接入首字段聚焦、Tab 循环、Escape 关闭和入口回焦；Agent 权限确认默认聚焦拒绝，Escape 等同拒绝并回收触发焦点；共享焦点 hook 过滤 inert/隐藏区域。完成相关单元、真实新建任务 Electron 场景及完整 69 条桌面回归（67 项通过、2 项按环境跳过） |
| 2026-09-01 | v2.115 | 延伸 Things / Todoist 的桌面连续操作：筛选和批量预览打开后进入首个可操作项，Tab 留在当前确认层，Escape/取消回到触发点，批量执行成功后回到批量入口；动态审计详情、设置确认层和 Todo Pet 右键菜单也统一首焦点与回焦。共享焦点路径排除 disabled、inert 与隐藏区域；完整单元回归 184 个文件、1209 项通过，完整桌面 Electron 回归 70 条中 68 项通过、2 项按环境跳过；手机端仍不在本轮范围 |
| 2026-09-01 | v2.116 | 延伸 Things / Todoist 的“打开即继续”桌面节奏：任务详情依赖链可以直接打开当前集合之外的前置/后续任务，慢速读取不会覆盖随后选择；循环任务范围确认绑定字段修订，确认保存后正确收敛脏状态；详情自动保存显示“有未保存编辑/正在保存…”状态，并保留失败后的本地草稿。完整单元回归 184 个文件、1210 项通过，完整桌面 Electron 回归 72 条中 70 项通过、2 项按环境跳过；手机端仍不在本轮范围 |
| 2026-09-01 | v2.117 | 记录桌面任务详情的在途保存关闭竞态：详情关闭后，已发出的异步保存完成不会重新打开详情，保存结果仍保留；对应测试入口为 [`tests/task-controller.test.tsx`](../tests/task-controller.test.tsx#L486) 中的 `does not reopen the inspector when an in-flight save finishes after close`。本条仅覆盖桌面端，手机端不在范围；当前回归基线为 **184 个 Vitest 文件、1212 项通过**，完整桌面 Electron 回归为 **72 条中 70 项通过、2 项按环境跳过** |
