...
19. 受限任务自动化：吸收 Trello Butler、ClickUp Rules 和 Kanboard 自动动作的“事件 → 条件 → 动作”结构。设置允许最多 50 条本机规则，触发为新建、完成、用户手动应用或按计划执行；条件编辑器可用来源、项目、清单、分组、标签和情境组成可读的 AND 组合，项目/清单优先显示本地实体名称；按计划规则支持每天或每周指定日期的本机时间，并用周期 checkpoint 防止重复。动作只更新重点标记、私有项目/清单/分组、稍后日期、标签和情境，复用普通更新与撤销；定时规则只处理开放任务。规则没有脚本、网络、任务创建或远端写回能力，执行串行且失败可解释。跨设备规则同步仍留在后续范围。
