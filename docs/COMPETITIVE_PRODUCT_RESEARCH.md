...
19. 受限任务自动化：吸收 Trello Butler、ClickUp Rules 和 Kanboard 自动动作的“事件 → 条件 → 动作”结构。设置允许最多 50 条本机规则，触发仅为新建或完成，条件和动作来自严格白名单；动作只更新重点标记、私有项目/清单/分组、稍后日期、标签和情境，复用普通更新与撤销。规则没有脚本、网络、任务创建或远端写回能力，执行串行且失败可解释。
