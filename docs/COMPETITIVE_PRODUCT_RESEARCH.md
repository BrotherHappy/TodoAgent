...
1. 声明式 JSON 动作包已支持安装、启用、更新和卸载；设置新增可视化动作包编辑器，可加载当前包、组合内置待机动作并安装启用，编辑器与 JSON 入口共用同一字段白名单和校验器。工作流模板已支持内置流程和安全 JSON 导入，模板只能描述任务字段和相对日期，不携带脚本、网络请求或任意文件操作。季节小事件已落地为可关闭的本地日期装饰与温和提示，只改变宠物外观和气泡，不影响任务、成长或同步；颜色主题包已扩展为本机四色编辑器加受限本地 PNG/JPEG/WebP 背景，仍不接受 SVG、网络地址、图片路径、脚本或任意 CSS；第三方连接器和网络素材市场仍列为后续扩展。
