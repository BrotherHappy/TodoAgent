...
6. 宠物日记、共同经历卡、轻量装扮、共同收藏图鉴、共同旅程章节和小窝；允许导出和删除。收藏图鉴只投影已有奖励库存，显示已解锁物品、数量和解锁提示；旅程章节只投影项目任务的完成比例和下一步，不创建影子任务或第二套成长计数。任务详情现可显式把一项任务写入共同日记，保留原任务链接，不复制任务，也不把私人备注带入日记正文。
