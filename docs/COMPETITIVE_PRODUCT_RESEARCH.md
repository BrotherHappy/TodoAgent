...
20. 统一本机查找：吸收 ClickUp 的“任务、文档、聊天、历史统一搜索”和 Notion / Joplin 的上下文检索习惯。`⌘/Ctrl+Shift+F` 或命令面板可在同一入口搜索任务标题、备注、私人备注、标签、情境、评论、研究卡、附件/链接元数据、自定义字段，以及项目、清单和本机会话；标题精确匹配优先，多个关键词必须同时满足，结果显示来源、状态和可读片段。选择任务回到 All 并定位原任务，选择会话直达 Agent 历史，项目/清单进入对应工作区。索引只来自本机已保存快照，不读取附件正文、不上传、不改任务或飞书；搜索失败可重试，空查询不展示隐式内容。
